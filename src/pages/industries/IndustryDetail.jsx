...
import { ArrowUpRight, ChevronDown, ChevronRight, Workflow, BookOpen, CheckCircle2 } from "lucide-react";
...
import { industriesContent } from "../../data/industriesContent.js";
import { INDUSTRY_ANIMATIONS } from "../../data/animations.js";
...
// A slug can be a top-level industry, or a trade page nested under a hub
// industry (one with `children`, e.g. Salons & Spas -> Salon). Resolve
// both, and keep track of the parent hub so breadcrumbs and "back to hub"
// links can reflect the real site structure.
function findIndustryMeta(slug) {
  const top = industries.find((i) => i.slug === slug);
  if (top) return { meta: top, parent: null };
  for (const hub of industries) {
    if (!hub.children) continue;
    const child = hub.children.find((c) => c.slug === slug);
    if (child) return { meta: child, parent: hub };
  }
  return { meta: null, parent: null };
}

export default function IndustryDetail() {
  const { slug } = useParams();
  const { meta: industryMeta, parent } = findIndustryMeta(slug);
  const content = industriesContent[slug];
  const path = `/industries/${slug}`;

  const breadcrumbBase = [
    { label: "Industries", path: "/industries" },
    ...(parent ? [{ label: parent.label, path: `/industries/${parent.slug}` }] : []),
  ];

  if (!content || !industryMeta) {
    return (
      <Placeholder
        title={industryMeta?.label || "Industry"}
        breadcrumb={[...breadcrumbBase, { label: industryMeta?.label || "Industry" }]}
        path={`/industries/${slug}`}
      />
    );
  }

  const Icon = INDUSTRY_ICONS[slug] || Workflow;
  const trail = [...breadcrumbBase, { label: industryMeta.label, path }];
    ...
    <>
      <Seo
        ...
        jsonLd={[
          ...
          serviceSchema({ name: content.h1, description: content.metaDescription, path, serviceType: industryMeta.label }),
        ...
        <div className="relative max-w-3xl">
          <nav aria-label="Breadcrumb" className="flex items-center gap-1.5 mb-5 text-xs" style={fontBody}>
            ...
            {parent && (
              <>
                <ChevronRight className="w-3 h-3 text-white/30" />
                <Link to={`/industries/${parent.slug}`} className="text-white/40 hover:text-white transition-colors">{parent.label}</Link>
              </>
            )}
            <ChevronRight className="w-3 h-3 text-white/30" />
            <span className="text-white/80">{industryMeta.label}</span>
          </nav>
          <div className="w-12 h-12 rounded-xl bg-white/10 flex items-center justify-center mb-5">
            <Icon className="w-6 h-6 text-[#7FC1FF]" />
          </div>
          <span className="text-[#7FC1FF] text-eyebrow" style={fontMono}>{industryMeta.label}</span>
        ...
        <section className="section-padding bg-white">
          <div className="max-w-3xl mx-auto space-y-16">
            ...
            {content.differentiator && (
              <div>
                <Reveal>
                  <span className="text-[#1E5FA8] text-eyebrow" style={fontMono}>Not just a chatbot</span>
        ...
        <section className="section-padding bg-[#0B2E5C]">
          <div className="max-w-6xl mx-auto grid lg:grid-cols-[1fr_340px] gap-12 items-start">
            ...
            <Reveal className="lg:sticky lg:top-28">
              <PageAnimation type={INDUSTRY_ANIMATIONS[slug]} light />
            </Reveal>
          </div>
        </section>
      )}
      ...
      {content.scale && (
        <section className="section-padding bg-[#EAF4FD]">
          <div className="max-w-3xl mx-auto">
            <Reveal>
              <span className="text-[#1E5FA8] text-eyebrow" style={fontMono}>Built for this, not adapted</span>
        ...
        <div className="max-w-4xl mx-auto text-center">
          <Reveal>
            <h2 style={fontDisplay} className="text-h2 text-white mb-6">
              {content.finalCta?.heading || "Ready to see this running for your business?"}
            </h2>
          ...
          {(relevantSolutions.length > 0) && (
            <Reveal className="mt-16 pt-10 border-t border-white/10 text-left">
              <h3 style={fontDisplay} className="text-white/80 text-sm uppercase tracking-tight mb-4">Related solutions</h3>
              <div className="flex flex-wrap gap-3">
                ...
                <Link to="/resources/faqs" className="inline-flex items-center gap-1.5 border border-white/20 hover:border-[#7FC1FF]/60 hover:bg-white/5 text-white text-sm px-4 py-2 rounded-full transition-colors">
